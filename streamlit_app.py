# app.py — Diabetes Prediction Web App
...
with col1:
    ...
    blood_pres  = st.slider("Blood Pressure (mm Hg)",
                              min_value=24, max_value=122, value=70)
    skin_thick  = st.slider("Skin Thickness (mm)",
                              min_value=7, max_value=99, value=23)

with col2:
    insulin     = st.slider("Insulin (μU/mL)",
                              min_value=14, max_value=846, value=80)
    bmi         = st.slider("BMI (kg/m²)",
                              min_value=18, max_value=67, value=32)
    dpf         = st.slider("Diabetes Pedigree Function",
                              min_value=0.08, max_value=2.42,
